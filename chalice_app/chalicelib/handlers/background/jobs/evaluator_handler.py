...
from chalicelib.data_objects import ChatMessage, EvaluationMessage
from chalicelib.retrieval_metrics import RetrievalMetrics, RetrievalMetricsResult
...
class EvaluationScores:
    ...
    # Optional LLM-based metrics (only for sampled requests)
    ...
    # Retrieval metrics (computed from reranker scores)
...
# Initialize once at module level (reused across Lambda invocations)
config = AppConfig()
langsmith_client = Client(
    api_key=config.langsmith_api_key, api_url=config.langsmith_api_url
)

# Use DeepSeek for cost-effective evaluation
...
def process_evaluation_task(eval_message: EvaluationMessage) -> None:
    ...
    # Get run_id from metadata (passed directly, no query needed)
    run_id = metadata.get("run_id")

    if not run_id:
        logger.warning(
            f"No run_id provided for request {request_id}, skipping feedback"
        )
        return

    # Post feedback directly to the run
    try:
        logger.info(f"Posting feedback to run_id: {run_id}")

        # Define which feedbacks to post based on available scores
        feedbacks_to_post = [
            # Overall score
            FeedbackEntry(
                key="overall_score",
                score=scores.overall_score,
                comment=f"Weighted average of all evaluation metrics (faithfulness 40%, actionability 35%, retrieval 25%). Tier: {scores.evaluation_tier}",
            ),
            # Heuristic score
            FeedbackEntry(
                key="heuristic_score",
                score=scores.heuristic_score,
                comment=f"Fast heuristic checks for product mentions, brand names, and response length. Details: {json.dumps({'has_products': scores.has_products, 'has_specifics': scores.has_specifics, 'response_length': scores.response_length})}",
            ),
        ]

        # Add LLM-based metrics if available
            ...
            feedbacks_to_post.extend(
                [
                    FeedbackEntry(
                        ...
                        comment="How well the response is grounded in provided Reddit context (1.0 = fully grounded, 0.0 = hallucinations).",
                    ),
                    FeedbackEntry(
            ...
            feedbacks_to_post.extend(
                [
                    FeedbackEntry(
                        ...
                        comment="How actionable and specific the product recommendations are (1.0 = highly actionable, 0.0 = vague).",
                    ),
                    FeedbackEntry(
            ...
            feedbacks_to_post.extend(
                [
                    FeedbackEntry(
                        ...
                        comment="How relevant the retrieved Reddit documents are to the user's query (1.0 = highly relevant, 0.0 = not relevant).",
                    ),
                    FeedbackEntry(
            ...
            feedbacks_to_post.extend(
                [
                    FeedbackEntry(
                        ...
                        comment="Proportion of relevant docs (per reranker) found in top 5 retrieval results",
                    ),
                    FeedbackEntry(
                        key="recall_at_10",
                        score=scores.recall_at_10,
                        comment="Proportion of relevant docs (per reranker) found in top 10 retrieval results",
                    ),
                    FeedbackEntry(
                        key="recall_at_15",
                        score=scores.recall_at_15,
                        comment="Proportion of relevant docs (per reranker) found in top 15 retrieval results",
                    ),
                    FeedbackEntry(
                        key="ndcg_at_5",
                        score=scores.ndcg_at_5,
                        comment="Normalized Discounted Cumulative Gain at 5 - measures ranking quality with position discount",
                    ),
                    FeedbackEntry(
                        key="ndcg_at_10",
                        score=scores.ndcg_at_10,
                        comment="Normalized Discounted Cumulative Gain at 10 - measures ranking quality with position discount",
                    ),
                    FeedbackEntry(
                        key="ndcg_at_15",
                        score=scores.ndcg_at_15,
                        comment="Normalized Discounted Cumulative Gain at 15 - measures ranking quality with position discount",
                    ),
                    FeedbackEntry(
                        key="mrr",
                        score=scores.mrr,
                        comment="Mean Reciprocal Rank - 1/position of first relevant document (higher = better)",
                    ),
                    FeedbackEntry(
                        key="hit_rate_at_5",
                        score=scores.hit_rate_at_5,
                        comment="Binary: Did we find any relevant doc in top 5? (1.0 = yes, 0.0 = no)",
                    ),
                    FeedbackEntry(
                        key="hit_rate_at_10",
                        score=scores.hit_rate_at_10,
                        comment="Binary: Did we find any relevant doc in top 10? (1.0 = yes, 0.0 = no)",
                    ),
                    FeedbackEntry(
                        key="hit_rate_at_15",
                        score=scores.hit_rate_at_15,
                        comment="Binary: Did we find any relevant doc in top 15? (1.0 = yes, 0.0 = no)",
                    ),
                ]
            )
        ...
        for feedback in feedbacks_to_post:
            try:
                ...
                logger.warning(f"Failed to post {feedback.key} feedback: {e}")

        logger.info(
            f"Posted {posted_count}/{len(feedbacks_to_post)} feedbacks for run {run_id}, overall: {scores.overall_score:.3f}"
        )
    except Exception as ls_error:
        logger.error(f"LangSmith feedback error for run {run_id}: {ls_error}")
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    logger.info(f"Received {len(event.get('Records', []))} evaluation tasks")
    ...
    for record in event.get("Records", []):
        try:
            ...
            logger.error(f"Error evaluating record: {e}", exc_info=True)
            failed += 1

    logger.info(f"Evaluation complete: {processed} processed, {failed} failed")
...
def run_comprehensive_evaluation(
    ...
    # ALWAYS RUN: Fast heuristics (no LLM cost)
    ...
    # RUN FOR 100%: All LLM-based evaluations
    try:
        logger.info(f"Running full LLM evaluation for request {request_id}")
        ...
        # Core Metric #3: Retrieval Relevance
        if retrieved_docs:
    ...
    # Compute retrieval metrics if we have the necessary data
    try:
        ...
        if pre_rerank_results and reranker_scores:
            logger.info(f"Computing retrieval metrics for request {request_id}")
            retrieval_metrics_result = compute_retrieval_metrics(
                pre_rerank_results, reranker_scores
            )
    except Exception as e:
        logger.error(f"Retrieval metrics computation failed: {e}", exc_info=True)

    # Build scores for computing overall score
...
def run_heuristic_checks(response: str) -> HeuristicResult:
    ...
    # Count capitalized words (likely product/brand names)
...
def evaluate_faithfulness(
    ...
    messages = [
        ...
        ChatMessage(
            ...
            content=FAITHFULNESS_USER_PROMPT.format(
                query=query, context=context_preview, response=response
            ),
        ),
    ]
    ...
    try:
        ...
        logger.warning(f"Failed to parse faithfulness response: {result}")
        return FaithfulnessResult(
...
def evaluate_actionability_llm(query: str, response: str) -> ActionabilityResult:
    ...
    try:
        ...
        logger.warning(f"Failed to parse actionability response: {result}")
        return ActionabilityResult(
...
def evaluate_retrieval_relevance(
    ...
    # Show top 3 retrieved docs to judge
    ...
    try:
        ...
        logger.warning(f"Failed to parse retrieval response: {result}")
...
def compute_retrieval_metrics(
    ...
    Args:
        pre_rerank_results: Results before reranking (RetrievalMetricsDocument or dict)
        reranker_scores: Relevance scores from reranker (RerankerJudgment or dict)
...
def compute_overall_score(scores: ScoresForComputation) -> float:
    ...
    # Prioritize the 3 core metrics when available
    core_scores = []

    if scores.faithfulness is not None:
        core_scores.append(scores.faithfulness * 0.4)  # 40% weight - most important

    if scores.actionability_llm is not None:
        core_scores.append(scores.actionability_llm * 0.35)  # 35% weight

    if scores.retrieval_relevance is not None:
        core_scores.append(scores.retrieval_relevance * 0.25)  # 25% weight

    # If we have core metrics, use weighted average
    if core_scores:
        return sum(core_scores)

    # Otherwise fall back to heuristic score
